...
class BinaryErrorDriftDescriptor():
    ...
    A drift is said to begin at the moment of the first detected warning that was followed by an
    uninterrupted series of warnings until the moment drift detected signal is detected from
    ...
    def update(self, x):
        ...
        if self.ddm.drift_detected:
            if len(self.error_history) >= (self.rate_calculation_sample_size * 2):
                ...
                error_rate_at_warning = sum(
                    self.error_history[:self.rate_calculation_sample_size]) / self.rate_calculation_sample_size
                error_rate_at_detection = sum(
                    self.error_history[-self.rate_calculation_sample_size:]) / self.rate_calculation_sample_size
