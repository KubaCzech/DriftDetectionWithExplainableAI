...
def cluster_windows(matrix: pd.DataFrame, fix_outliers = True):
    ...
    if fix_outliers:
        # replace outliers (-1) with assignments of its neighbors if they are the same
...
def clustered_labels_accuracy(labels, true_concept, outliers_included = True):
    ...
    accuracy_with_outliers = accuracy_no_outliers * (1-(n_outliers/len(labels))) # assume outliers are always incorrectly classified
