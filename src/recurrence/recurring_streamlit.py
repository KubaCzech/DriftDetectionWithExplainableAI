...
from visualisation import plot_prototype_comparison
        ...
        try:
            ...
            if drift_positions_str.strip():
                try:
                    ...
                    st.error("Invalid drift positions format. Please enter comma-separated numbers (e.g., '28000, 52000, 70000')")
                    st.stop()
        ...
        for class_name in sorted(prototypes.keys()):
            with st.expander(f"📌 Class {class_name} - {len(prototypes[class_name])} prototypes"):
                ...
                for proto_idx, prototype in enumerate(proto_list):
                    # Calculate distances from this prototype to all samples
                    distances = []
                    for sample in x_dicts:
                    ...
                    for i, sample in enumerate(x_dicts):
                        # Check if this prototype is closest among all prototypes of this class
