...
# Adjust imports for dashboard structure
        ...
        if st.button("🚀 Process Data", type="primary", key="race_process_btn"):
            try:
                ...
                for i in range(num_windows):
                    ...
                    # Ensure we don't go out of bounds (though range handles it)
                ...
                for i, (x_block, y_block) in enumerate(dataset_chunks):
                    ...
                    # Learn from data (incremental learning)
                    # Convert to dict for river if needed, assume numpy array for now
                    # APete expects dicts probably? River models usually expect dicts.
                    # Let's check format. ARFClassifier learn_one expects x as dict.
                    ...
                    # Create prototypes
                    # APete explainer using the current state of the model
                    current_explainer = APete(model=model, alpha=0.01)
                    
                    # select_prototypes expects x as list of dicts or numpy? 
                    # Looking at recurring_streamlit.py:
                    # ds.take gives tuples. River generators usually give dicts naturally. 
                    # So x_block probably needs to be list of dicts if passing to river-based APete?
                    # Let's assume passed x_dicts is correct.
                    # Wait, APete from protree... let's check its usage. 
                    # In recurring_streamlit.py: ds.take returns points. 
                    # It creates dataset as list of [x_block, y_block].
                    # x_block is whatever river generator yields. Usually dicts.
                    
                    current_prototypes = current_explainer.select_prototypes(x_dicts, y_list)
                    
                    # Store window data
                    # FullWindowStorage expects x as raw data (dicts likely), y as labels
    ...
    with tab2:
        ...
        if not st.session_state.race_processing_complete:
            ...
            if len(unique_labels) > 0:
                ...
                cluster_color_map = {l: cmap(i % 10) for i, l in enumerate(real_clusters)}
